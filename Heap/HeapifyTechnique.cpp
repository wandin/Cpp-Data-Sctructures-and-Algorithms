...
template<class T>
void Heapify<T>::Heapifying(int n) {
    for (int i = n / 2; i >= 1; i--) {
        Adjust(i, n);
    }
}
...
template<class T>
void Heapify<T>::DelMax(int n, T &x) {
    if (n == 0) {
...
template<class T>
void Heapify<T>::Adjust(int i, int n) {
...
template<class T>
void Heapify<T>::HeapSort() {
    T x;
    Heapifying(n); // Build the heap
    for (int i = n; i >= 1; i--) {
...
int main() {
    try {
        Heapify<int> h(8); // Create a heap with 8 elements
        cout << "\n Before Sorting..." << endl;
